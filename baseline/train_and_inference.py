...
from utils.util_old import find_max_epoch, print_size, training_loss, calc_diffusion_hyperparams,calc_diffusion_hyperparams2
...
def train_inference(output_directory,
          ckpt_iter,
          n_iters,
          iters_per_ckpt,
          iters_per_logging,
    ...
    while n_iter < n_iters + 1:
        for batch in training_data:
            ...
            if n_iter > 0 and n_iter % (iters_per_ckpt*2) == 0:
                ...
                torch.save({'model_state_dict': net.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(),'loss':loss.item()},
                           os.path.join(final_directory, checkpoint_name))
                print('model at iteration %s is backup saved' % n_iter)
    ...
    print('Model saved at:', output_directory)
    print('Backup saved at:', final_directory)
    ...
    if trainset_config['train_data_path'] is not None:    
        if trainset_config['test_data_path'] == "./datasets/Mujoco/test_mujoco.npy":
            ...
            testing_data = DataLoader(elec_test_dataset, batch_size=500, shuffle=False)
            print('Data loaded',flush=True)
...
if __name__ == "__main__":
    ...
    global gen_config
    gen_config = config["gen_config"]  # generation parameters
